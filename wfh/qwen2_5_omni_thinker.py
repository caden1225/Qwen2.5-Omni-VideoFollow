...
class Qwen2_5OmniThinkerProcessingInfo(Qwen2AudioProcessingInfo,
    ...
    def get_max_audio_tokens(self) -> int:
        # Match actual audio processing output: 375 tokens
        return 375

    def get_max_image_tokens(self) -> int:
        # Match actual image processing output: 256 tokens
        return 256

    def get_max_video_tokens(self, seq_len: int, mm_counts: Mapping[str, int]) -> int:
        # Match actual video processing output: based on empirical testing
        # Current video processing gives 256 tokens
        return 256

    def get_image_size_with_most_features(self):
        # Return smaller image size for memory efficiency
        from vllm.multimodal.parse import ImageSize
        # Use 448x448 instead of max possible size
        return ImageSize(width=448, height=448)

    def get_num_frames_with_most_features(self, seq_len: int, mm_counts: Mapping[str, int]) -> int:
        # Limit video frames for memory efficiency
        return min(4, max(1, seq_len // 1024))
...
class Qwen2_5OmniThinkerDummyInputsBuilder(
    ...
    def _get_dummy_processor_inputs(self, seq_len: int, num_audios: int,
        ...
        # Use smaller audio length for memory efficiency
        target_audio_length = min(
            feature_extractor.chunk_length // 2,  # Reduce by half
            15,  # Reduce from 30 to 15 seconds
        ) * feature_extractor.sampling_rate
        
        # Use optimized smaller image size
        target_width, target_height = \
            self.info.get_image_size_with_most_features()
        
        # Use optimized smaller frame count
        target_num_frames = \
        ...
        hf_processor_mm_kwargs = {
            'videos_kwargs': {
                'min_pixels' : 3136,  # 56*56, keep minimum
                'max_pixels' : 200704,  # 448*448, match our optimized size
            }
        }
...
class Qwen2_5OmniThinkerMultiModalProcessor(
    ...
    def _call_hf_processor(
        ...
        # Handle both 'audio' and 'audios' keys for compatibility
        ...
        if ('input_audio_features' not in hf_inputs
            ...
            if feature_attention_mask is not None:
                # Apply attention mask while preserving correct dimensions
                # Original: [batch, mel_bins, time]
                batch_size, mel_bins, time_len = input_features.shape
                mask = feature_attention_mask.bool()  # [batch, time]
                
                # Process each item in batch separately 
                processed_features = []
                for i in range(batch_size):
                    # Get valid time steps for this batch item
                    valid_time_steps = mask[i].sum().item()
                    # Extract and reshape: [mel_bins, valid_time]
                    item_features = input_features[i, :, :valid_time_steps]
                    processed_features.append(item_features)
                
                # For now, assume batch_size=1 and use first item
                input_features = processed_features[0]  # [mel_bins, valid_time]
...
class Qwen2_5OmniConditionalGenerationMixin:
    ...
    def _process_audio_input(
        ...
        if input_features.ndim == 3:
            ...
            # Already in correct shape [mel_bins, time] from our processing
            pass
        if audio_feature_lengths.ndim == 2:
            # Handle multiple batch sizes, not just 1
            if audio_feature_lengths.shape[0] == 1:
                audio_feature_lengths = audio_feature_lengths.squeeze(0)
            else:
                # For multiple batches, take the first one as representative
                audio_feature_lengths = audio_feature_lengths[0]
